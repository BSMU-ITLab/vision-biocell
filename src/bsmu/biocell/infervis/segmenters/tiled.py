...
class MultipassTiledMdiSegmenter(MdiSegmenter):
    ...
    def segment_async(
        ...
        layered_data, raster = self._check_duplicate_mask_and_get_active_layered_data(
    ...
    def update_mask_layer_partially(
        ...
        mask_layer = layered_data.layer_by_name(mask_layer_name, RasterLayer)
        if mask_layer is None:
            raise RuntimeError(
                f'Cannot partially update mask layer `{mask_layer_name}`: '
                f'raster layer with this name does not exist.'
            )
        is_foreground_class = mask == self.mask_foreground_class
        if modifiable_mask is not None:
            is_foreground_class &= modifiable_mask
        mask_layer.raster_pixels[is_foreground_class] = self.mask_foreground_class
        mask_layer.data.emit_pixels_modified()
    ...
    def update_mask_layer(
        ...
        if mask_layer is not None and not isinstance(mask_layer, RasterLayer):
            raise TypeError(
                f'Cannot update mask layer `{mask_layer_name}`: '
                f'layer exists but is `{type(mask_layer).__name__}`, not `RasterLayer`.'
            )
        ...
        if mask_draw_mode == MaskDrawMode.REDRAW_ALL or mask_layer is None or not mask_layer.is_raster_pixels_valid:
            ...
            is_modified = mask_layer.raster_pixels == self.mask_background_class
            mask_layer.raster_pixels[is_modified] = mask[is_modified]
            mask_layer.data.emit_pixels_modified()
        else:
